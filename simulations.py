...
# Sample command c_w,c_winners = bu.association_sim()
def associate_multi(n=100000,k=317,p=0.05,beta=0.1,overlap_iter=10):
	b = brain.Brain(p,save_winners=True)
	b.add_stimulus("stimA",k)
	b.add_area("A",n,k,beta)
	b.add_stimulus("stimB",k)
	...
	for i in range(0, 9):
		b.project({"stimA":["A"]},
			{"A":["A","D"],"D":["D"]}) 
	# Project B->D
	b.project({"stimB":["B"]},{"B":["B","D"]}) #edited
	for i in range(0, 9):
		b.project({"stimB":["B"]},
			{"B":["B","D"],"D":["D"]})
	# Project C->D
	b.project({"stimB":["C"]},{"C":["C","D"]}) #edited
	for i in range(0, 9):
		b.project({"stimB":["C"]},
			{"C":["C","D"],"D":["D"]})
	...
	# Project just B (not sure)
	b.project({"stimB":["B"]},{"B":["B","D"]})
	for i in range(0, 9):
		b.project({"stimB":["B"]},{"B":["B","D"],"D":["D"]})
	return b

def association_multi_sim(n=100000,k=317,p=0.05,beta=0.1,overlap_iter=10):
	b = associate(n,k,p,beta,overlap_iter) 
	return b.areas["D"].saved_w,b.areas["D"].saved_winners

def association_multi_grand_sim(n=100000,k=317,p=0.01,beta=0.05,min_iter=10,max_iter=20):
	b = brain.Brain(p,save_winners=True)
	b.add_stimulus("stimA",k)
	b.add_area("A",n,k,beta)
	b.add_stimulus("stimB",k)
	...
	for i in range(0, min_iter,max_iter+1):
		...
		b_copy2.project({"stimB":["B"]},{})
		b_copy2.project({},{"B":["D"]})
		# in copy 3, project just B
		b_copy3.project({"stimC":["C"]},{})
		b_copy3.project({},{"C":["D"]})
		o = bu.overlap(b_copy1.areas["D"].winners, b_copy2.areas["D"].winners)
		results[i] = float(o)/float(k)
	return results
...
def associate_multiple_areas(n=100000,k=317,p=0.05,beta=0.1,overlap_iter=10,areas=2):
	...
	for key, value in area_dict.items():
		area_dict[key].append(target_area)
